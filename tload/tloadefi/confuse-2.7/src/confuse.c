...
static cfg_opt_t *cfg_dupopt_array(cfg_opt_t *opts)
{
    ...
    dupopts = (cfg_opt_t *)calloc(n+1, sizeof(cfg_opt_t));
...
#ifdef DEBUG
    printf("cfg: cfg_init_defaults: cfg @ %p, opts @ %p\n", cfg, &cfg->opts[0]);
#endif

    for(i = 0; cfg->opts[i].name; i++)
    {
#ifdef DEBUG
	    printf("cfg: cfg_init_defaults: %d, %s, %p, %d\n", i, cfg->opts[i].name,
			    &cfg->opts[i].name[0], cfg->opts[i].type);
#endif
        ...
        {
#ifdef DEBUG
		printf("cfg: cfg_init_defaults: is CFGT_SEC && !CFGF_MULTI\n");
#endif
...
#ifdef DEBUG
	    printf("cfg: cfg_init_defaults: %d, %s ends\n", i, cfg->opts[i].name);
#endif
    }
}
...
cfg_setopt(cfg_t *cfg, cfg_opt_t *opt, char *value)
{
    ...
    else
    {
		 ...
		 * non-NULL section title. */
                ...
                for(idx = 0; idx < opt->nvalues && val == NULL; idx++)
                {
                    cfg_t *sec = opt->values[idx]->section;
    ...
    switch(opt->type)
    {
        ...
        case CFGT_SEC:
#ifdef DEBUG
	    printf("cfg: cfg_setopt: CFGT_SEC\n");
#endif
            if(is_set(CFGF_MULTI, opt->flags) || val->section == 0)
            {
#ifdef DEBUG
		    printf("cfg: cfg_setopt: CFGF_MULTI\n");
#endif
