...
#define GC_CURTIME_T int
#define GC_CURTIME_GETMS(pcurt) (0)
...
GC_API void *GC_CALL GC_malloc_int(size_t size)
{
 struct GC_gcdata_s *gcdata;
 return GC_general_malloc(&gcdata, (GC_word)size, ~(GC_word)0);
}

/* The following three functions are added for tysos */
static int gc_global_lock = 0;

void lock(volatile int *l)
{
	while(__sync_lock_test_and_set(l, 1))
	{
		// The builtin for some reason doesn't work
